...
/// Stub AI import. Stands in for the real Anthropic-backed parser until the
/// backend ships (#76): it ignores the uploaded bytes and returns a
/// representative parsed draft per file after a short delay so the
/// review-and-save flow is fully exercisable.
///
// TODO(#76): replace with HttpRecipeImportService in main.dart wiring.
class LocalRecipeImportService implements RecipeImportService {
  ...
  Future<Recipe> parse({
    ...
    final results = await parseAll([
      RecipeImportFile(bytes: bytes, filename: filename, contentType: ''),
    ]);
    return results.first.draft!;
  }
  ...
  Future<List<RecipeImportResult>> parseAll(
    ...
    return [
      for (final f in files)
        RecipeImportResult(
          filename: f.filename,
          tier: f.filename.toLowerCase().endsWith('.json') ? 'json' : 'sonnet',
          draft: _draftFor(f.filename),
        ),
    ];
  }

  Recipe _draftFor(String filename) => Recipe(
        id: 'staging',
